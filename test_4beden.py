"""4 beden testi — tüm DXF'ler için, amaç: kumaş boyunu minimize et."""
import glob, os, sys
import numpy as np
import time
...
print(f"{'='*70}")
print(f"4 BEDEN NESTİNG BENCHMARKİ")
print(f"{'='*70}")
print(f"Kumaş eni: {BIN_WIDTH}mm")
print(f"Beden sayısı: {N_BEDEN}")
...
for dxf_path in dxf_files:
    ...
    # 1 beden yükle
    pieces_1 = load_dxf(dxf_path)
    n_1 = len(pieces_1)
    area_1 = sum(p["area"] for p in pieces_1)

    # 4 beden — parçaları kopyala
    pieces = []
    ...
    min_length = total_area / BIN_WIDTH  # teorik min (mm)

    print(f"--- {name} ---")
    print(f"  1 beden: {n_1} parça, {area_1/100:.0f} cm²")
    ...
    print(f"  Rastgele: {baseline['utilization']:.1f}%, {baseline['used_length']:.0f}mm, yerleşen: {baseline['n_placed']}/{n}")

    # GA+SA
    np.random.seed(42)
    t0 = time.time()
    result = ga_sa_hybrid(decoder.fitness, n, pop_size=POP, max_iter=ITER, verbose=False)
...
# Özet tablo
print(f"\n{'='*70}")
print(f"{'MODEL':<25} {'PARÇA':>6} {'TEORİK':>8} {'RAST.':>6} {'GA+SA':>6} {'UZUNLUK':>9} {'FİRE':>7}")
print(f"{'='*70}")
for r in results:
    print(f"{r['model']:<25} {r['parcalar_4beden']:>6} {r['teorik_min_mm']:>7.0f}mm {r['rastgele_pct']:>5.1f}% {r['gasa_pct']:>5.1f}% {r['gasa_uzunluk_mm']:>8.0f}mm {r['fire_m']:>6.2f}m")
print(f"{'='*70}")
